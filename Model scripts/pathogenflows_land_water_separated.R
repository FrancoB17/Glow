...
# }
  ...
  for(i in 1:length(emissions$subarea)){
    ...
    #MAYBE BETTER TO MOVE THIS TO LATER AND REPLACE BY THE 'OUT' EMISSIONS, currently then calculating twice the same thing.
      ...
      emissions$to_sewerage_flushSewer_urb_out_water[i]<-emissions$pathogen_urb_conforgrid_sewer[i]*HUMAN_DATA$removalfraction[i]
      emissions$to_sewerage_flushSewer_rur_out_water[i]<-emissions$pathogen_rur_conforgrid_sewer[i]*HUMAN_DATA$removalfraction[i]
      emissions$to_fecalSludge_flushSeptic_urb_out_water[i]<-emissions$to_fecalSludge_flushSeptic_urb[i]*HUMAN_DATA$removalfraction[i]
      emissions$to_fecalSludge_flushPit_urb_out_water[i]<-emissions$to_fecalSludge_flushPit_urb[i]*HUMAN_DATA$removalfraction[i]
      emissions$to_fecalSludge_flushOpen_urb_out_water[i]<-emissions$to_fecalSludge_flushOpen_urb[i]*HUMAN_DATA$removalfraction[i]
      emissions$to_fecalSludge_flushUnknown_urb_out_water[i]<-emissions$to_fecalSludge_flushUnknown_urb[i]*HUMAN_DATA$removalfraction[i]
      emissions$to_fecalSludge_pitSlab_urb_out_water[i]<-emissions$to_fecalSludge_pitSlab_urb[i]*HUMAN_DATA$removalfraction[i]
      emissions$to_fecalSludge_pitNoSlab_urb_out_water[i]<-emissions$to_fecalSludge_pitNoSlab_urb[i]*HUMAN_DATA$removalfraction[i]
...
#      emissions$to_fecalSludge_compostingTwinNoSlab_urb_out[i]<-emissions$to_fecalSludge_compostingTwinNoSlab_urb[i]*HUMAN_DATA$removalfraction[i]
      emissions$to_fecalSludge_compostingToilet_urb_out_water[i]<-emissions$to_fecalSludge_compostingToilet_urb[i]*HUMAN_DATA$removalfraction[i]
      emissions$to_fecalSludge_bucketLatrine_urb_out_water[i]<-emissions$to_fecalSludge_bucketLatrine_urb[i]*HUMAN_DATA$removalfraction[i]
      emissions$to_fecalSludge_containerBased_urb_out_water[i]<-emissions$to_fecalSludge_containerBased_urb[i]*HUMAN_DATA$removalfraction[i]
      emissions$to_fecalSludge_other_urb_out_water[i]<-emissions$to_fecalSludge_other_urb[i]*HUMAN_DATA$removalfraction[i]
      emissions$to_fecalSludge_flushSeptic_rur_out_water[i]<-emissions$to_fecalSludge_flushSeptic_rur[i]*HUMAN_DATA$removalfraction[i]
      emissions$to_fecalSludge_flushPit_rur_out_water[i]<-emissions$to_fecalSludge_flushPit_rur[i]*HUMAN_DATA$removalfraction[i]
      emissions$to_fecalSludge_flushOpen_rur_out_water[i]<-emissions$to_fecalSludge_flushOpen_rur[i]*HUMAN_DATA$removalfraction[i]
      emissions$to_fecalSludge_flushUnknown_rur_out_water[i]<-emissions$to_fecalSludge_flushUnknown_rur[i]*HUMAN_DATA$removalfraction[i]
      emissions$to_fecalSludge_pitSlab_rur_out_water[i]<-emissions$to_fecalSludge_pitSlab_rur[i]*HUMAN_DATA$removalfraction[i]
      emissions$to_fecalSludge_pitNoSlab_rur_out_water[i]<-emissions$to_fecalSludge_pitNoSlab_rur[i]*HUMAN_DATA$removalfraction[i]
...
#      emissions$to_fecalSludge_compostingTwinNoSlab_rur_out[i]<-emissions$to_fecalSludge_compostingTwinNoSlab_rur[i]*HUMAN_DATA$removalfraction[i]
      emissions$to_fecalSludge_compostingToilet_rur_out_water[i]<-emissions$to_fecalSludge_compostingToilet_rur[i]*HUMAN_DATA$removalfraction[i]
      emissions$to_fecalSludge_bucketLatrine_rur_out_water[i]<-emissions$to_fecalSludge_bucketLatrine_rur[i]*HUMAN_DATA$removalfraction[i]
      emissions$to_fecalSludge_containerBased_rur_out_water[i]<-emissions$to_fecalSludge_containerBased_rur[i]*HUMAN_DATA$removalfraction[i]
      emissions$to_fecalSludge_other_rur_out_water[i]<-emissions$to_fecalSludge_other_rur[i]*HUMAN_DATA$removalfraction[i]
  ...
  emissions$to_surface_flushSewer_urb_out_water<-emissions$to_surface_flushSewer_urb
  emissions$to_surface_flushSeptic_urb_out_water<-emissions$to_surface_flushSeptic_urb*(1-HUMAN_DATA$onsiteDumpedland_urb)
  emissions$to_surface_flushPit_urb_out_water<-emissions$to_surface_flushPit_urb*(1-HUMAN_DATA$onsiteDumpedland_urb)
  emissions$to_surface_flushOpen_urb_out_water<-emissions$to_surface_flushOpen_urb
  emissions$to_surface_flushUnknown_urb_out_water<-emissions$to_surface_flushUnknown_urb
  emissions$to_surface_pitSlab_urb_out_water<-emissions$to_surface_pitSlab_urb*(1-HUMAN_DATA$onsiteDumpedland_urb)
  emissions$to_surface_pitNoSlab_urb_out_water<-emissions$to_surface_pitNoSlab_urb*(1-HUMAN_DATA$onsiteDumpedland_urb)
  ...
  emissions$to_surface_compostingToilet_urb_out_water<-emissions$to_surface_compostingToilet_urb*(1-HUMAN_DATA$onsiteDumpedland_urb)
  emissions$to_surface_bucketLatrine_urb_out_water<-emissions$to_surface_bucketLatrine_urb
  emissions$to_surface_containerBased_urb_out_water<-emissions$to_surface_containerBased_urb
  emissions$to_surface_hangingToilet_urb_out_water<-emissions$to_surface_hangingToilet_urb
  emissions$to_surface_openDefecation_urb_out_water<-emissions$to_surface_openDefecation_urb
  emissions$to_surface_other_urb_out_water<-emissions$to_surface_other_urb
  
  emissions$to_surface_flushSewer_rur_out_water<-emissions$to_surface_flushSewer_rur
  emissions$to_surface_flushSeptic_rur_out_water<-emissions$to_surface_flushSeptic_rur*(1-HUMAN_DATA$onsiteDumpedland_rur)
  emissions$to_surface_flushPit_rur_out_water<-emissions$to_surface_flushPit_rur*(1-HUMAN_DATA$onsiteDumpedland_rur)
  emissions$to_surface_flushOpen_rur_out_water<-emissions$to_surface_flushOpen_rur
  emissions$to_surface_flushUnknown_rur_out_water<-emissions$to_surface_flushUnknown_rur
  emissions$to_surface_pitSlab_rur_out_water<-emissions$to_surface_pitSlab_rur*(1-HUMAN_DATA$onsiteDumpedland_rur)
  emissions$to_surface_pitNoSlab_rur_out_water<-emissions$to_surface_pitNoSlab_rur*(1-HUMAN_DATA$onsiteDumpedland_rur)
  ...
  emissions$to_surface_compostingToilet_rur_out_water<-emissions$to_surface_compostingToilet_rur*(1-HUMAN_DATA$onsiteDumpedland_rur)
  emissions$to_surface_bucketLatrine_rur_out_water<-0
  emissions$to_surface_containerBased_rur_out_water<-0
  emissions$to_surface_hangingToilet_rur_out_water<-emissions$to_surface_hangingToilet_rur
  emissions$to_surface_openDefecation_rur_out_water<-0
  emissions$to_surface_other_rur_out_water<-emissions$to_surface_other_rur
  
  emissions$to_surface_flushSewer_urb_out_land<-0
  emissions$to_surface_flushSeptic_urb_out_land<-emissions$to_surface_flushSeptic_urb*HUMAN_DATA$onsiteDumpedland_urb*0.025
  emissions$to_surface_flushPit_urb_out_land<-emissions$to_surface_flushPit_urb*HUMAN_DATA$onsiteDumpedland_urb*0.025
  emissions$to_surface_flushOpen_urb_out_land<-0
  emissions$to_surface_flushUnknown_urb_out_land<-0
  emissions$to_surface_pitSlab_urb_out_land<-emissions$to_surface_pitSlab_urb*HUMAN_DATA$onsiteDumpedland_urb*0.025
  emissions$to_surface_pitNoSlab_urb_out_land<-emissions$to_surface_pitNoSlab_urb*HUMAN_DATA$onsiteDumpedland_urb*0.025
  #emissions$to_surface_compostingTwinSlab_urb_out<-emissions$to_surface_compostingTwinSlab_urb*(1-HUMAN_DATA$onsiteDumpedland_urb)+emissions$to_surface_compostingTwinSlab_urb*HUMAN_DATA$onsiteDumpedland_urb*0.025
  #emissions$to_surface_compostingTwinNoSlab_urb_out<-emissions$to_surface_compostingTwinNoSlab_urb*(1-HUMAN_DATA$onsiteDumpedland_urb)+emissions$to_surface_compostingTwinNoSlab_urb*HUMAN_DATA$onsiteDumpedland_urb*0.025
  emissions$to_surface_compostingToilet_urb_out_land<-emissions$to_surface_compostingToilet_urb*HUMAN_DATA$onsiteDumpedland_urb*0.025
  emissions$to_surface_bucketLatrine_urb_out_land<-0
  emissions$to_surface_containerBased_urb_out_land<-0
  emissions$to_surface_hangingToilet_urb_out_land<-0
  emissions$to_surface_openDefecation_urb_out_land<-0
  emissions$to_surface_other_urb_out_land<-0
  
  emissions$to_surface_flushSewer_rur_out_land<-0
  emissions$to_surface_flushSeptic_rur_out_land<-emissions$to_surface_flushSeptic_rur*HUMAN_DATA$onsiteDumpedland_rur*0.025
  emissions$to_surface_flushPit_rur_out_land<-emissions$to_surface_flushPit_rur*HUMAN_DATA$onsiteDumpedland_rur*0.025
  emissions$to_surface_flushOpen_rur_out_land<-0
  emissions$to_surface_flushUnknown_rur_out_land<-0
  emissions$to_surface_pitSlab_rur_out_land<-emissions$to_surface_pitSlab_rur*HUMAN_DATA$onsiteDumpedland_rur*0.025
  emissions$to_surface_pitNoSlab_rur_out_land<-emissions$to_surface_pitNoSlab_rur*HUMAN_DATA$onsiteDumpedland_rur*0.025
  #emissions$to_surface_compostingTwinSlab_rur_out<-emissions$to_surface_compostingTwinSlab_rur*(1-HUMAN_DATA$onsiteDumpedland_rur)+emissions$to_surface_compostingTwinSlab_rur*HUMAN_DATA$onsiteDumpedland_rur*0.025
  #emissions$to_surface_compostingTwinNoSlab_rur_out<-emissions$to_surface_compostingTwinNoSlab_rur*(1-HUMAN_DATA$onsiteDumpedland_rur)+emissions$to_surface_compostingTwinNoSlab_rur*HUMAN_DATA$onsiteDumpedland_rur*0.025
  emissions$to_surface_compostingToilet_rur_out_land<-emissions$to_surface_compostingToilet_rur*HUMAN_DATA$onsiteDumpedland_rur*0.025
  emissions$to_surface_bucketLatrine_rur_out_land<-emissions$to_surface_bucketLatrine_rur*0.025
  emissions$to_surface_containerBased_rur_out_land<-emissions$to_surface_containerBased_rur*0.025
  emissions$to_surface_hangingToilet_rur_out_land<-0
  emissions$to_surface_openDefecation_rur_out_land<-emissions$to_surface_openDefecation_rur*0.025
  emissions$to_surface_other_rur_out_land<-0
  
  #####Simulate fraction of each sanitation type of total emissions (going to land)
  
  fractions <- data.frame(iso=HUMAN_DATA$iso,subarea=HUMAN_DATA$subarea,pathogen_urb_landforgrid=emissions$pathogen_urb_landforgrid,pathogen_rur_landforgrid=emissions$pathogen_rur_landforgrid)  
  
  #urban fractions
  fractions$fland_urb_flushSewer<-emissions$to_surface_flushSewer_urb_out_land/emissions$pathogen_urb_landforgrid
  fractions$fland_urb_flushSeptic<- emissions$to_surface_flushSeptic_urb_out_land/emissions$pathogen_urb_landforgrid
  fractions$fland_urb_flushPit <- emissions$to_surface_flushPit_urb_out_land/emissions$pathogen_urb_landforgrid
  fractions$fland_urb_flushOpen<-emissions$to_surface_flushOpen_urb_out_land/emissions$pathogen_urb_landforgrid
  fractions$fland_urb_flushUnknown<-emissions$to_surface_flushUnknown_urb_out_land/emissions$pathogen_urb_landforgrid
  fractions$fland_urb_pitSlab <- emissions$to_surface_pitSlab_urb_out_land/emissions$pathogen_urb_landforgrid
  fractions$fland_urb_pitNoSlab <- emissions$to_surface_pitNoSlab_urb_out_land/emissions$pathogen_urb_landforgrid
  fractions$fland_urb_compostingToilet <- emissions$to_surface_compostingToilet_urb_out_land/emissions$pathogen_urb_landforgrid 
  fractions$fland_urb_bucketLatrine<-emissions$to_surface_bucketLatrine_urb_out_land/emissions$pathogen_urb_landforgrid
  fractions$fland_urb_containerBased<-emissions$to_surface_containerBased_urb_out_land/emissions$pathogen_urb_landforgrid
  fractions$fland_urb_openDefecation<-emissions$to_surface_openDefecation_urb_out_land/emissions$pathogen_urb_landforgrid
  fractions$fland_urb_hangingToilet<-emissions$to_surface_hangingToilet_urb_out_land/emissions$pathogen_urb_landforgrid
  fractions$fland_urb_other<-emissions$to_surface_other_urb_out_land/emissions$pathogen_urb_landforgrid
  
  #rural fractions
  fractions$fland_rur_flushSewer<- emissions$to_surface_flushSewer_rur_out_land/emissions$pathogen_rur_landforgrid
  fractions$fland_rur_flushSeptic<- emissions$to_surface_flushSeptic_rur_out_land/emissions$pathogen_rur_landforgrid
  fractions$fland_rur_flushPit <- emissions$to_surface_flushPit_rur_out_land/emissions$pathogen_rur_landforgrid
  fractions$fland_rur_flushOpen<-emissions$to_surface_flushOpen_rur_out_land/emissions$pathogen_rur_landforgrid
  fractions$fland_rur_flushUnknown<-emissions$to_surface_flushUnknown_rur_out_land/emissions$pathogen_rur_landforgrid
  fractions$fland_rur_pitSlab <- emissions$to_surface_pitSlab_rur_out_land/emissions$pathogen_rur_landforgrid
  fractions$fland_rur_pitNoSlab <- emissions$to_surface_pitNoSlab_rur_out_land/emissions$pathogen_rur_landforgrid
  fractions$fland_rur_compostingToilet<- emissions$to_surface_compostingToilet_rur_out_land/emissions$pathogen_rur_landforgrid
  fractions$fland_rur_bucketLatrine<-emissions$to_surface_bucketLatrine_rur_out_land/emissions$pathogen_rur_landforgrid
  fractions$fland_rur_containerBased<-emissions$to_surface_containerBased_rur_out_land/emissions$pathogen_rur_landforgrid
  fractions$fland_rur_hangingToilet<-emissions$to_surface_hangingToilet_rur_out_land/emissions$pathogen_rur_landforgrid
  fractions$fland_rur_openDefecation<-emissions$to_surface_openDefecation_rur_out_land/emissions$pathogen_rur_landforgrid
  fractions$fland_rur_other<-emissions$to_surface_other_rur_out_land/emissions$pathogen_rur_landforgrid
  
  out_csv <- file.path(SCENARIO$model_output,sprintf("landfractions_%s_%s.csv",PATHOGEN$name,SCENARIO$run))
  write.csv(fractions,out_csv)
...
pathogenflow.calc.totals <- function(emissions){
  ...
  totals$total_flushSewer_out_water<- rowSums(cbind(emissions$to_sewerage_flushSewer_urb_out_water, emissions$to_surface_flushSewer_urb_out_water, emissions$to_sewerage_flushSewer_rur_out_water, emissions$to_surface_flushSewer_rur_out_water),na.rm=T)
  totals$total_flushSeptic_out_water<- rowSums(cbind(emissions$to_fecalSludge_flushSeptic_urb_out_water,emissions$to_surface_flushSeptic_urb_out_water,emissions$to_fecalSludge_flushSeptic_rur_out_water,emissions$to_surface_flushSeptic_rur_out_water), na.rm=T)
  totals$total_flushPit_out_water<- rowSums(cbind(emissions$to_fecalSludge_flushPit_urb_out_water,emissions$to_surface_flushPit_urb_out_water,emissions$to_fecalSludge_flushPit_rur_out_water,emissions$to_surface_flushPit_rur_out_water), na.rm=T)
  totals$total_flushOpen_out_water<- rowSums(cbind(emissions$to_fecalSludge_flushOpen_urb_out_water,emissions$to_surface_flushOpen_urb_out_water,emissions$to_fecalSludge_flushOpen_rur_out_water,emissions$to_surface_flushOpen_rur_out_water), na.rm=T)
  totals$total_flushUnknown_out_water<-rowSums(cbind(emissions$to_fecalSludge_flushUnknown_urb_out_water,emissions$to_surface_flushUnknown_urb_out_water,emissions$to_fecalSludge_flushUnknown_rur_out_water,emissions$to_surface_flushUnknown_rur_out_water), na.rm=T)
  totals$total_pitSlab_out_water<- rowSums(cbind(emissions$to_fecalSludge_pitSlab_urb_out_water,emissions$to_surface_pitSlab_urb_out_water,emissions$to_fecalSludge_pitSlab_rur_out_water,emissions$to_surface_pitSlab_rur_out_water), na.rm = T)
  totals$total_pitNoSlab_out_water<-rowSums(cbind(emissions$to_fecalSludge_pitNoSlab_urb_out_water,emissions$to_surface_pitNoSlab_urb_out_water,emissions$to_fecalSludge_pitNoSlab_rur_out_water,emissions$to_surface_pitNoSlab_rur_out_water), na.rm=T)
  #totals$total_compostingTwinSlab_out_water<- emissions$to_fecalSludge_compostingTwinSlab_urb_out_water,emissions$to_surface_compostingTwinSlab_urb_out_water,emissions$to_fecalSludge_compostingTwinSlab_rur_out_water,emissions$to_surface_compostingTwinSlab_rur_out_water
  #totals$total_compostingTwinNoSlab_out_water<- emissions$to_fecalSludge_compostingTwinNoSlab_urb_out_water,emissions$to_surface_compostingTwinNoSlab_urb_out_water,emissions$to_fecalSludge_compostingTwinNoSlab_rur_out_water,emissions$to_surface_compostingTwinNoSlab_rur_out_water
  totals$total_compostingToilet_out_water<- rowSums(cbind(emissions$to_fecalSludge_compostingToilet_urb_out_water,emissions$to_surface_compostingToilet_urb_out_water,emissions$to_fecalSludge_compostingToilet_rur_out_water,emissions$to_surface_compostingToilet_rur_out_water), na.rm = T)
  totals$total_bucketLatrine_out_water<- rowSums(cbind(emissions$to_fecalSludge_bucketLatrine_urb_out_water,emissions$to_surface_bucketLatrine_urb_out_water,emissions$to_fecalSludge_bucketLatrine_rur_out_water,emissions$to_surface_bucketLatrine_rur_out_water), na.rm=T)
  totals$total_containerBased_out_water<- rowSums(cbind(emissions$to_fecalSludge_containerBased_urb_out_water,emissions$to_surface_containerBased_urb_out_water,emissions$to_fecalSludge_containerBased_rur_out_water,emissions$to_surface_containerBased_rur_out_water), na.rm = T)
  totals$total_hangingToilet_out_water<- rowSums(cbind(emissions$to_surface_hangingToilet_urb_out_water,emissions$to_surface_hangingToilet_rur_out_water), na.rm = T)
  totals$total_openDefecation_out_water<- rowSums(cbind(emissions$to_surface_openDefecation_urb_out_water,emissions$to_surface_openDefecation_rur_out_water), na.rm = T)
  totals$total_other_out_water<- rowSums(cbind(emissions$to_fecalSludge_other_urb_out_water,emissions$to_surface_other_urb_out_water,emissions$to_fecalSludge_other_rur_out_water,emissions$to_surface_other_rur_out_water), na.rm = T)
  ...
  totals$total_water<-NA
  for(i in 1:length(totals$iso)){
    totals$total_water[i]<-sum(c(totals$total_flushSewer_out_water[i],totals$total_flushSeptic_out_water[i],totals$total_flushPit_out_water[i],totals$total_flushOpen_out_water[i],totals$total_flushUnknown_out_water[i],totals$total_pitSlab_out_water[i],totals$total_pitNoSlab_out_water[i],totals$total_compostingToilet_out_water[i],totals$total_bucketLatrine_out_water[i],totals$total_containerBased_out_water[i],totals$total_hangingToilet_out_water[i],totals$total_openDefecation_out_water[i],totals$total_other_out_water[i]),na.rm=TRUE)
  }
  
  for(i in 1:length(totals$iso)){
    if(sum(as.numeric(totals[i,3:15]),na.rm=TRUE)==0 && sum(as.numeric(HUMAN_DATA[i,21:33]),na.rm=TRUE)+sum(as.numeric(HUMAN_DATA[i,44:56]),na.rm=TRUE)==0){
      totals$total_water[i]<-NA   
    }  
  }
  
  totals$total1_water<-log10(totals$total_water)
  a<-which(totals$total1_water=="-Inf")
  totals$total1_water[a]<-0
  totals$total_water_log<-totals$total1_water
  
  totals$total1_water<-NULL
  
  totals$total_flushSewer_out_land<- rowSums(cbind(emissions$to_surface_flushSewer_urb_out_land, emissions$to_surface_flushSewer_rur_out_land),na.rm=T)
  totals$total_flushSeptic_out_land<- rowSums(cbind(emissions$to_surface_flushSeptic_urb_out_land,emissions$to_surface_flushSeptic_rur_out_land), na.rm=T)
  totals$total_flushPit_out_land<- rowSums(cbind(emissions$to_surface_flushPit_urb_out_land,emissions$to_surface_flushPit_rur_out_land), na.rm=T)
  totals$total_flushOpen_out_land<- rowSums(cbind(emissions$to_surface_flushOpen_urb_out_land,emissions$to_surface_flushOpen_rur_out_land), na.rm=T)
  totals$total_flushUnknown_out_land<-rowSums(cbind(emissions$to_surface_flushUnknown_urb_out_land,emissions$to_surface_flushUnknown_rur_out_land), na.rm=T)
  totals$total_pitSlab_out_land<- rowSums(cbind(emissions$to_surface_pitSlab_urb_out_land,emissions$to_surface_pitSlab_rur_out_land), na.rm = T)
  totals$total_pitNoSlab_out_land<-rowSums(cbind(emissions$to_surface_pitNoSlab_urb_out_land,emissions$to_surface_pitNoSlab_rur_out_land), na.rm=T)
  #totals$total_compostingTwinSlab_out_land<- emissions$to_fecalSludge_compostingTwinSlab_urb_out_land,emissions$to_surface_compostingTwinSlab_urb_out_land,emissions$to_fecalSludge_compostingTwinSlab_rur_out_land,emissions$to_surface_compostingTwinSlab_rur_out_land
  #totals$total_compostingTwinNoSlab_out_land<- emissions$to_fecalSludge_compostingTwinNoSlab_urb_out_land,emissions$to_surface_compostingTwinNoSlab_urb_out_land,emissions$to_fecalSludge_compostingTwinNoSlab_rur_out_land,emissions$to_surface_compostingTwinNoSlab_rur_out_land
  totals$total_compostingToilet_out_land<- rowSums(cbind(emissions$to_surface_compostingToilet_urb_out_land,emissions$to_surface_compostingToilet_rur_out_land), na.rm = T)
  totals$total_bucketLatrine_out_land<- rowSums(cbind(emissions$to_surface_bucketLatrine_urb_out_land,emissions$to_surface_bucketLatrine_rur_out_land), na.rm=T)
  totals$total_containerBased_out_land<- rowSums(cbind(emissions$to_surface_containerBased_urb_out_land,emissions$to_surface_containerBased_rur_out_land), na.rm = T)
  totals$total_hangingToilet_out_land<- rowSums(cbind(emissions$to_surface_hangingToilet_urb_out_land,emissions$to_surface_hangingToilet_rur_out_land), na.rm = T)
  totals$total_openDefecation_out_land<- rowSums(cbind(emissions$to_surface_openDefecation_urb_out_land,emissions$to_surface_openDefecation_rur_out_land), na.rm = T)
  totals$total_other_out_land<- rowSums(cbind(emissions$to_surface_other_urb_out_land,emissions$to_surface_other_rur_out_land), na.rm = T)
  
  for (i in 1:length(HUMAN_DATA$iso)){
    if(sum(as.numeric(totals[i,18:30]),na.rm=TRUE)==0 && sum(as.numeric(HUMAN_DATA[i,21:33]),na.rm=TRUE)+sum(as.numeric(HUMAN_DATA[i,44:56]),na.rm=TRUE)==0){
      totals[i,18:30]<-NA      
    }
  }
  
  totals$total_land<-NA
  for(i in 1:length(totals$iso)){
    totals$total_land[i]<-sum(c(totals$total_flushSewer_out_land[i],totals$total_flushSeptic_out_land[i],totals$total_flushPit_out_land[i],totals$total_flushOpen_out_land[i],totals$total_flushUnknown_out_land[i],totals$total_pitSlab_out_land[i],totals$total_pitNoSlab_out_land[i],totals$total_compostingToilet_out_land[i],totals$total_bucketLatrine_out_land[i],totals$total_containerBased_out_land[i],totals$total_hangingToilet_out_land[i],totals$total_openDefecation_out_land[i],totals$total_other_out_land[i]),na.rm=TRUE)
  }
  
  for(i in 1:length(totals$iso)){
    if(sum(as.numeric(totals[i,18:30]),na.rm=TRUE)==0 && sum(as.numeric(HUMAN_DATA[i,21:33]),na.rm=TRUE)+sum(as.numeric(HUMAN_DATA[i,44:56]),na.rm=TRUE)==0){
      totals$total_land[i]<-NA   
    }  
  }
  
  totals$total1_land<-log10(totals$total_land)
  a<-which(totals$total1_land=="-Inf")
  totals$total1_land[a]<-0
  totals$total_land_log<-totals$total1_land
  
  totals$total1_land<-NULL
