  ...
  result = withCallingHandlers({
    ...
    if(SCENARIO$loadings_module==1){
      ...
      if(SCENARIO$hydrology_available){
        source("./Model scripts/pathogenflows_land_water_separated.R")
      }else{
...
#    else 
    ...
    if(SCENARIO$gadm_level!=4){
      ...
      if(SCENARIO$hydrology_available){
        #separately write emissions to land and to water before runoff fraction is taken into account
        outfile_land<-file.path(SCENARIO$model_output,sprintf("humanemissions_diffuse_%s_%s.tif",PATHOGEN$name,SCENARIO$run))
        outfile_water<-file.path(SCENARIO$model_output,sprintf("humanemissions_water_%s_%s.tif",PATHOGEN$name,SCENARIO$run))
        writeRaster(OUTPUT$grid$pathogen_land,outfile_land,format="GTiff",overwrite=TRUE)
        writeRaster(OUTPUT$grid$pathogen_water,outfile_water,format="GTiff",overwrite=TRUE)
      }
      OUTPUT$grid$pathogen_land <<-0.025*OUTPUT$grid$pathogen_land
      OUTPUT$grid$pathogen_water <<- OUTPUT$grid$pathogen_water + OUTPUT$grid$pathogen_land
      OUTPUT$grid$pathogen_water[OUTPUT$grid$pathogen_water==0]<-NA
      out_file <- file.path(SCENARIO$model_output,sprintf("humanemissions_total_%s_%s.tif",PATHOGEN$name,SCENARIO$run))
